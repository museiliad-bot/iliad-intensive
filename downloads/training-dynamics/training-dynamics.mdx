...
**Exercise 4.6 (The balanced function-space ODE).** Substitute the results of [Exercise 4.4](#ex-td-2cii) and [4.5](#ex-td-2ciii) into [Exercise 4.3](#ex-td-2ci) to obtain:
...
**Setup.** Consider a single mode: a depth-2 diagonal DLN with scalar weights $a, b$ learning a target $s > 0$. From [Section 4](#4-gradient-flow-and-conserved-quantities) and [5](#5-rich-regime-saddle-to-saddle), the balanced gradient flow for $w = ab$ is:
