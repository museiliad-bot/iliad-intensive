...
<YouTube id="IHHrJdZKICs" title="The Dumbest Neural Network Worth Studying" />

<YouTube id="clTLzFD2uC8" title="Uncovering The Hidden Biases of Deep Learning" />
...
**Exercise 3.3 (Critical-point structure).** Achour et al. ([Achour et al. 2024](#bib-achour2024)) show that every first-order critical point $\theta = (W_{1}, \ldots, W_{L})$ satisfies: there exists a subset $S \subseteq \{1, \ldots, d\}$ such that
...
From now on, assume **balanced initialization**: $G(0) = 0$, which by [Exercise 4.2](#ex-td-2b) means $W_{2}^{\top} W_{2} = W_{1} W_{1}^{\top}$ for all time. We want to derive the gradient flow in function space (the ODE for the student $W = W_{2} W_{1}$). This requires several steps.

<Exercise id="ex-td-2ci">
**Exercise 4.3 (Function-space velocity).** Compute $\dot{W}:= \frac{d}{dt}(W_{2} W_{1})$ using the gradient flow equations from [Exercise 4.1](#ex-td-2a). Show that:
...
Analogous to [Exercise 4.4](#ex-td-2cii).

</Solution>

<Exercise id="ex-td-2civ">
**Exercise 4.6 (The balanced function-space ODE).** Substitute the results of [Exercises 4.4](#ex-td-2cii) and [4.5](#ex-td-2ciii) into [Exercise 4.3](#ex-td-2ci) to obtain:
...
Substituting into [Exercise 4.3](#ex-td-2ci):
...
Show that the gradient flow from [Exercise 4.6](#ex-td-2civ) can be written as $\dot{W}= K[M - W]$.
...
With $F = M - W$, the equation from [Exercise 4.6](#ex-td-2civ) reads:
...
This is the core problem. We derive the exact solution of the rich regime dynamics directly from the self-consistent equation of [Section 4](#4-gradient-flow-and-conserved-quantities), emphasizing the NTK perspective.

**Alignment assumption.** Start from the balanced gradient flow equation derived in [Exercise 4.6](#ex-td-2civ):
...
**Connection to the saddle structure ([Exercise 3.3](#ex-td-1c)):** The saddle points $P_{S} M$ with $|S| = k$ are exactly the best rank-$k$ approximations to $M$, with loss $\frac{1}{2}\sum_{\alpha > k}s_{\alpha}^{2}$. The gradient flow trajectory passes **near** these saddles as it incrementally recruits modes. The **plateaus** in the loss curve correspond to time spent near saddle points (only $k$ modes active), and the **sharp transitions** correspond to escaping along the unstable direction that activates the next mode. $\square$

</Solution>

## 6. Lazy regime

We now show that large initialization freezes the NTK and eliminates the timescale separation found in [Section 5](#5-rich-regime-saddle-to-saddle). For clarity, we work with the **diagonal scalar** model from [Exercise 3.1](#ex-td-1a), so each mode $\alpha$ is independent. This avoids matrix algebra and isolates the essential mechanism.

**Setup.** Consider a single mode: a depth-2 diagonal DLN with scalar weights $a, b$ learning a target $s > 0$. From [Sections 4](#4-gradient-flow-and-conserved-quantities) and [5](#5-rich-regime-saddle-to-saddle), the balanced gradient flow for $w = ab$ is:
...
**Exercise 6.2 (Frozen-NTK solution).** Solve the linearized ODE from [Exercise 6.1](#ex-td-4a). Show that:
...
This is **exponential** convergence to $s$ (contrast with the sigmoid of [Section 5](#5-rich-regime-saddle-to-saddle)). The learning timescale is:
...
**Exercise 6.3 (No timescale separation).** Now restore the mode index $\alpha$. In the lazy regime, each mode satisfies $\dot{w}_{\alpha} \approx 2w_{0}(s_{\alpha} - w_{\alpha})$ with the **same** rate $2w_{0}$ for all $\alpha$. Compare the lazy learning time $t_{\alpha}^{\text{lazy}}$ with the rich-regime timescale from [Exercise 5.3](#ex-td-3b): $t_{\alpha}^{\text{rich}}\propto 1/s_{\alpha}$.
</Exercise>
...
For $w_{\alpha} > 0$ this is $2w_{\alpha}(s_{\alpha} - w_{\alpha})$, exactly the logistic ODE from [Exercise 5.2](#ex-td-3aii). $\square$
...
This is a linear ODE with rate $2\tau$, independent of $s_{\alpha}$ — the frozen-NTK regime of [Section 6](#6-lazy-regime) (with $\tau$ playing the role of $w_{0}$). $\square$
...
- The strong default suggestion for everyone is: Saxe et al. [A mathematical theory of semantic development in deep neural networks](https://www.pnas.org/doi/10.1073/pnas.1820226116) — Read the supplementary materials section to understand the exact solution of gradient flow dynamics in deep linear networks
...
- To go further: [Alternating Gradient Flows: A Theory of Feature Learning in Two-layer Neural Networks](https://arxiv.org/abs/2506.06489)
