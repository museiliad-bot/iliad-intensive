...
2. To understand the local learning coefficient via volume scaling: Read [Subsection 3.1](#31-the-local-learning-coefficient-via-volume-scaling-asymptotics) and complete [Exercise 3.1](#ex-llc-regular-case), [3.2](#ex-quad-valley-scaling), [3.4](#ex-calculating-llc) and [3.7](#ex-cubic-parameterisation). Read [Subsection 3.3](#33-local-learning-coefficients-of-deep-linear-networks) and [Exercise 3.10](#ex-dln-lc).
...
**(c)** From [Exercise 2.3(d)](#ex-product-degeneracy), the coordinate directions $(1, 0)$ and $(0, 1)$ are degenerate precisely on the coordinate axes: $(1, 0)$ is degenerate on $\{b = 0\}$ and $(0, 1)$ is degenerate on $\{a = 0\}$. This matches exactly: the axis $\{b = 0\}$ is where the $a$-translation symmetry acts, providing the degenerate direction $(1, 0)$, and the axis $\{a = 0\}$ is where the $b$-translation symmetry acts, providing $(0, 1)$.
...
**(d)** Show the converse: if the directional score $s_{v}(x, y, w)$ vanishes for all $x \in \X$ and $y \in \Y$ at $w$, then $\Psi$ is degenerate at $w$ in direction $v$.
</Exercise>
...
**(b)** If $D_{v} \Psi(w) = 0$ for some non-zero $v$, then by [Exercise 2.11(c)](#ex-score-properties), $s_{v}(x,y,w) = 0$ for all $x, y$. Let $\hat{v}= v / \|v\|$. Then $s_{\hat{v}}= s_{v}$ (the directional derivative is invariant to the magnitude of $v$), so by part (a), $\hat{v}^{\top} I(w)\, \hat{v}= \E[s_{\hat{v}}^{2}] = 0$. A matrix $M$ is positive definite only if $u^{\top} M u > 0$ for all non-zero $u$. Since $\hat{v}$ is a non-zero vector with $\hat{v}^{\top} I(w)\, \hat{v}= 0$, the matrix $I(w)$ is not positive definite.
