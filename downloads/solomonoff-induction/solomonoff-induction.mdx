...
- **Pareto optimality** ([Exercises 6.2](#prob-kl-pareto) and [8.2](#prob-sq-pareto)): no other predictor weakly dominates $\xi$ on every $\nu \in \Mc$, for either KL or squared loss.
...
*Step (1): Pinsker, pointwise.* For each $t$ and $x_{<t}$, both $\mu(\cdot \mid x_{<t})$ and $\xi(\cdot \mid x_{<t})$ are probability distributions on $\mathbb{B}$ ($\xi$ by [Exercise 1.2](#prob-one-step-ex)), so [Theorem 4.1](#thm-pinsker) gives
...
In [Section 6](#6-pareto-optimality-under-kl-loss) and [Section 8](#8-pareto-optimality-under-squared-loss) we establish that the Bayesian mixture $\xi$ is *uniquely Pareto-optimal* under both KL and squared prediction loss: no other predictor can do at least as well as $\xi$ on every environment $\nu \in \Mc$ without coinciding with $\xi$ entirely.
