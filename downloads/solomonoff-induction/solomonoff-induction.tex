...
\begin{itemize}\itemsep=2pt
  ...
  \item $xy$: string concatenation. if $x := x_{1:n}$ and $y := y_{1:m}$ then
  $xy = x_1 \ldots x_n y_1 \ldots y_m$. 
  \item $x_{i:j} := x_i x_{i+1} \ldots x_j$.
  \item $x_{<t} := x_1 x_2 \cdots x_{t-1}$.
...
\[
\nu(x_{1:n} | y) ~:=~ \prod_{t=1}^{n} \nu(x_t \mid y x_{<t}), \qquad \nu(\epsilon) := 1.
\]
The unconditional joint is defined as $\nu(x_{1:n}) := \nu(x_{1:n} \mid \epsilon)$.
The true (unknown) environment generating the data is denoted $\mu$.
\end{definition}
