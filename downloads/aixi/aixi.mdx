...
A good target is to complete [Sections 7](#7-on-policy-value-convergence-of-bayes) and [8](#8-aixi-cannot-be-fooled-in-deterministic-environments). The self-optimizing property ([Sections 9–11](#9-likelihood-ratios-are-martingales)) requires substantial additional machinery (supermartingales, change of measure) and is an advanced stretch goal.

**Critical path.** The three results share a common foundation ([Sections 0–4](#0-properties-of-measures)) and then diverge:

<Figure src="/uploads/aixi/tikz-4dc0e19d14b9.svg" alt="diagram" />
...
- $\cH \equiv \cH^{*}$: set of all finite histories
...
and hence, collecting the percept factors with the chain rule ([Exercise 0.2](#prob-chain-rule)) and taking $m \to \infty$ ([Exercises 2.5](#prob-inf-horizon) and [2.7](#prob-inf-opt-policy)), that AIXI selects the action
...
$$
\mu^{\pi}\!\Big(\Big\{\aes_{1:\infty}: V_{\xi}^{\pi}(\aes_{<t}) - V_{\mu}^{\pi}(\aes_{<t}) \not\to 0\Big\}\Big) = 0.
$$
