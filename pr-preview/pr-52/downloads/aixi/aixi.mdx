...
**Definition 7.2 (Covering[^4]).** We say $Q$ **covers** $P$ if $Q$ is positive everywhere $P$ is: $P(A) > 0 \implies Q(A) > 0$ for all events $A$. Equivalently: any event that $Q$ rules out, $P$ also rules out. By [Section 3](#3-dominance-of-the-bayesian-mixture), $\xi^{\pi}$ covers $\mu^{\pi}$.
...
[^1]: See [Appendix B](#b-worked-example-value-function) for a worked example. For simplicity, we consider only geometric discounting.
