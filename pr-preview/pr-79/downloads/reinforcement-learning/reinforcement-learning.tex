...
&=\; \sum_{a_t \in \cA} \pi(a_t \mid s)\! \sum_{s_{t+1} \in \cS} \!T(s_{t+1} \mid s, a_t)\! \sum_{a_{t+1} \in \cA} \pi(a_{t+1} \mid s_{t+1})\! \sum_{s_{t+2} \in \cS} \!T(s_{t+2} \mid s_{t+1}, a_{t+1})\, \cdots \\
&\qquad\qquad \bigl[\, R(s, a_t, s_{t+1}) + \gamma\, R(s_{t+1}, a_{t+1}, s_{t+2}) + \gamma^2\, R(s_{t+2}, a_{t+2}, s_{t+3}) + \cdots \,\bigr].
\end{align*}
...
\begin{align*}
V_\pi(s) &= \sum_{a \in \cA} \pi(a \mid s) \sum_{s' \in \cS} T(s' \mid s, a) \;\mathbb{E}_\pi\!\left[r_{t+1} + \gamma\, G_{t+1} \;\middle|\; s_t = s,\, a_t = a,\, s_{t+1} = s'\right].
\end{align*}
...
$$
\|V_{k+1} - V_k\|_\infty \;=\; \|\mathcal{B} V_k - \mathcal{B} V_{k-1}\|_\infty \;\le\; \gamma\, \|V_k - V_{k-1}\|_\infty \;\le\; \cdots \;\le\; \gamma^k\, \|V_1 - V_0\|_\infty.
$$
...
$$
\|V^* - V_n\|_\infty \;\le\; \sum_{k=n}^{\infty} \|V_{k+1} - V_k\|_\infty \;\le\; \sum_{k=n}^{\infty} \gamma^k\, \|V_1 - V_0\|_\infty \;=\; \frac{\gamma^n}{1-\gamma}\, \|V_1 - V_0\|_\infty.
$$
\end{solution}
...
\begin{align*}
\|V^* - V_n\|_\infty
&\le\; \|V_{n+1} - V_n\|_\infty \;+\; \|V_{n+2} - V_{n+1}\|_\infty \;+\; \|V_{n+3} - V_{n+2}\|_\infty \;+\; \cdots \\
&\le\; \gamma\, \|V_n - V_{n-1}\|_\infty \;+\; \gamma^2\, \|V_n - V_{n-1}\|_\infty \;+\; \gamma^3\, \|V_n - V_{n-1}\|_\infty \;+\; \cdots \\
...
&= \left|\sum_{s'} T(s' \mid s,a)\,\gamma\Bigl(\max_{a'} Q(s',a') - \max_{a'} Q'(s',a')\Bigr)\right| \\
&\le \sum_{s'} T(s' \mid s,a)\,\gamma\,\max_{a'}\bigl|Q(s',a') - Q'(s',a')\bigr| \\
&\le \sum_{s'} T(s' \mid s,a)\,\gamma\,\max_{a', s''}\bigl|Q(s'',a') - Q'(s'',a')\bigr| \\
...
&= \mathbb{E}\bigl[R(s_t, a_t, s_{t+1}) + \gamma \max_{a'} Q_t(s_{t+1}, a') \mid \mathcal{F}_t\bigr] - (\mathcal{B}Q_t)(s_t, a_t) \\
&= \sum_{s'} T(s' \mid s_t, a_t)\bigl[R(s_t, a_t, s') + \gamma \max_{a'} Q_t(s', a')\bigr] - (\mathcal{B}Q_t)(s_t, a_t) \\
&= (\mathcal{B}Q_t)(s_t, a_t) - (\mathcal{B}Q_t)(s_t, a_t) = 0.
\end{align*}
