...
\begin{itemize}
    ...
    \pause
    \item \textit{Outcome 2}: Recognise thedifference between preference, utility, and reward.
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \pause
    \begin{itemize}
        \exitem E.g.\ \textbf{dominance}: another policy yields prefered trajectories at least as preferred in every state of the world, and strictly so in at least one.
    \end{itemize}
    \pause
...
\begin{frame}
\frametitle{vNM representaiton: The big picture}
\begin{enumerate}
...
\vspace{0.25cm}
\textbf{Result:} 
The first two axioms give ranking structure based on a cardinal utility; 
the last two axioms make the utility to be an expected value.
\end{frame}
...
\end{itemize}
\vspace{0.4cm}
Classic counter-examples are lexicographic preferences, in which events can be 
infinitely different from each other. 
\vspace{0.4cm}

\textit{Example}: $\mu$ is eating chocolate, $\nu$ is eating potatos, and $\xi$ is accepting genocide.
\end{frame}
...
\begin{itemize}
    ...
    \item A 1-in-100 shot at \$100 \textit{is worst than} a 1-in-200 shota at \$250.
\end{itemize}
...
\begin{itemize}
    ...
    \item Inconmesurable gaps can exist
\end{itemize}
\end{column}
\end{columns}
...
\vspace{0.4cm}
\centering
vNM utility futher refines ordinal utility (it is not just a relabeling).
\end{frame}
...
\begin{itemize}
    ...
    \pause
    \item From a global utility $u(h)$, one can alsways define the increment
    $$
...
\begin{itemize}
    ...
    \pause
    \item Instead of `given a rewards, how to optimise it' we could ask `what kind of properties we want our reward to satisfy'. For example, Are any of these properties (or their negation) useful for fostering:
    \begin{itemize}
        \item Corregibility? (e.g. incompleteness)
        \pause
