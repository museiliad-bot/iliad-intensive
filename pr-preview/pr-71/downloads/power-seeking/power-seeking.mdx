...
cluster: D
day: D.6
title: "Power-Seeking: Exercises"
summary: >-
  Power-seeking as a theorem: how to count reward functions, which decision
  rules count as suitable, and what it means to keep more options open.
contributors:
...
the expectation taken over trajectories $s_{0}, s_{1}, s_{2}, \dots$ generated by $\pi$. The goal of reinforcement learning is to find an **optimal policy**, ie attaining the **optimal value**
...
- **(E)** embeds $a'$ into $a$: $\ \phi\cdot\F(s\mid a') \subseteq \F(s\mid a)$; and
- **(D)** is a symmetry of $\Dist$: $\ \phi_{*}\Dist = \Dist$.
...
$$
A'(\phi\cdot r) = \frac{N_{\phi\cdot r}(\F(s\mid a'))}{N_{\phi\cdot r}(\F(s))}= \frac{N_{r}(G)}{N_{\phi\cdot r}(\F(s))}, \ \ \  A(\phi\cdot r) \ge \frac{N_{\phi\cdot r}(G)}{N_{\phi\cdot r}(\F(s))}= \frac{N_{r}(\F(s\mid a'))}{N_{\phi\cdot r}(\F(s))},
$$
...
$$
\begin{aligned}\F(s\mid a')&= \{\, f' \,\},&f'&= e_{s} + \tfrac{\gamma}{1-\gamma}\,e_{B}, \\[2pt] \F(s\mid a)&= \{\, f_{G_1},\, f_{G_2}\,\},&f_{G_1}&= e_{s} + \tfrac{\gamma}{1-\gamma}\,e_{G_1}, \qquad f_{G_2}= e_{s} + \gamma\,e_{G_1}+ \tfrac{\gamma^2}{1-\gamma}\,e_{G_2}:\end{aligned}
$$
